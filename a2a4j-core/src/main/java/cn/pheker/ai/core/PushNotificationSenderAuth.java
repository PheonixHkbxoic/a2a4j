...
public class PushNotificationSenderAuth extends PushNotificationAuth {
    ...
    public void sendPushNotification(String url, Object data) {
        ...
        try (CloseableHttpClient http = HttpClients.createDefault()) {
            String token = this.generateJwt(data);

            HttpPost httpPost = new HttpPost(url);
            httpPost.addHeader(AUTH_HEADER, AUTH_HEADER_PREFIX + token);

            StringEntity stringEntity = new StringEntity(objectMapper.writeValueAsString(data), StandardCharsets.UTF_8.name());
            ...
            log.warn("Error during sending push-notification for URL: {}", url);
        }
    }
